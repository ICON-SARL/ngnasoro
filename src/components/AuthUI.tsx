...
const AuthUI = () => {
  ...
  useEffect(() => {
    if (user && !loading) {
      ...
      // Redirect based on role - adding timeout to allow roles to be properly loaded
      setTimeout(() => {
  ...
  // Utilisation d'un délai plus court pour l'écran de chargement
  if (loading && !authSuccess) {
    return (
