  ...
  ({ value, onChange, error, label, className, disabled, ...props }, ref) => {
    ...
    return (
      <div className="space-y-2">
        ...
        <div 
          className={cn(
            "flex items-stretch rounded-2xl overflow-hidden",
            "bg-background/60 backdrop-blur-sm",
            "border-2 transition-all duration-300",
            error 
              ? "border-destructive/50 focus-within:border-destructive" 
              : "border-border/40 focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10",
            disabled && "opacity-50 cursor-not-allowed",
            className
          )}
        >
          {/* Prefix with flag and country code */}
          <div className="flex items-center gap-2 px-4 py-4 bg-muted/40 border-r border-border/30 select-none">
            <span className="text-xl">{MALI_FLAG_EMOJI}</span>
