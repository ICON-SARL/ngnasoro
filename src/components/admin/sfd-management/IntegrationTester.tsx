...
export function IntegrationTester() {
  ...
  const [selectedTest, setSelectedTest] = useState<TestEndpoint>(testEndpoints[0]);
  const [requestPayload, setRequestPayload] = useState(JSON.stringify(testEndpoints[0].payload, null, 2));
  
  const { isLoading, testResult, errorMessage, runTest } = useIntegrationTester();
  ...
  const handleRunTest = async () => {
    ...
    // Run the test
    await runTest(selectedTest, requestPayload);
  };
