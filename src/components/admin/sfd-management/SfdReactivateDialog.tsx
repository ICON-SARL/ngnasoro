...
export function SfdReactivateDialog({
  ...
  const handleReactivate = async () => {
    ...
    setShowError(false);
    try {
      await reactivateSfdMutation.mutateAsync(selectedSfd.id);
      onOpenChange(false);
    } catch (error: any) {
        ...
        <DialogFooter className="px-4 py-3 bg-gray-50 border-t">
          <div className="flex justify-between w-full">
            ...
            <Button 
              ...
              disabled={reactivateSfdMutation.isPending || !selectedSfd}
              className="bg-green-600 hover:bg-green-700 h-8 text-sm"
              size="sm"
            >
              {reactivateSfdMutation.isPending ? 'Réactivation...' : 'Confirmer la réactivation'}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
