  ...
  return <div className="fixed inset-0 bg-white z-50 overflow-y-auto">
      <div className="p-4">
        ...
        <div className="space-y-6">
          {mobileMenuSections.map(section => <div key={section.id} className="flex flex-col space-y-1">
              <h3 className="text-sm font-medium text-gray-500 mb-2">{section.title}</h3>
              {section.items.map(item => <Button key={item.id} variant="ghost" className="justify-start" onClick={() => navigateAndClose(item.route)} disabled={item.coming}>
                  {getIcon(item.icon, `h-5 w-5 mr-2 ${item.color ? `text-[${item.color}]` : ""}`)} 
                  <span className="flex-1 text-base">{item.label}</span>
                  {item.coming && <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">Bientôt</span>}
                </Button>)}
            </div>)}
