...
// LOAN STATUS WEBHOOKS HANDLER - Consolidated from loan-status-webhooks function
const processLoanStatusChange = async (req: Request) => {
  try {
    const { loanId, status, event } = await req.json();
...
serve(async (req) => {
  ...
  // Handle loan status webhook - consolidated from loan-status-webhooks function
  if (req.method === 'POST' && pathParts[0] === 'loan-status-webhooks') {
